...
from PySide6.QtCore import (
    QAbstractTableModel, QDate, QEasingCurve, QFileInfo, QModelIndex,
    QPropertyAnimation, QSettings, QSize, QThread, QTimer, Qt, QUrl,
    Signal, QMimeData
)
from PySide6.QtGui import (
    QAction, QBrush, QColor, QDrag, QDragEnterEvent, QDropEvent, QFont,
    QImage, QIcon, QKeyEvent, QKeySequence, QPainter, QPixmap, QShortcut,
    QTextCharFormat, QTextCursor
)
from PySide6.QtWidgets import (
    QAbstractItemView, QApplication, QCalendarWidget, QCheckBox,
    QComboBox, QDialog, QDockWidget, QFileDialog, QFormLayout, QFrame,
    QGraphicsOpacityEffect, QGraphicsScene, QGraphicsView, QGridLayout,
    ...
    QPlainTextEdit, QProgressDialog, QPushButton, QRadioButton, QScrollArea,
    QSizePolicy, QSlider, QSpinBox, QSplitter, QStackedWidget, QStatusBar,
    QStyle, QTabWidget, QTableView, QTableWidget, QTableWidgetItem,
...
THEMES = {
    "Dark Nexus": """
        ...
        QTabBar::tab { background: #161b22; color: #8b949e; padding: 8px 15px; border: 1px solid #30363d; border-bottom: none; border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: 2px; }
        ...
        QMenu { background-color: #161b22; color: #c9d1d9; border: 1px solid #30363d; padding: 5px; }
        QMenu::item:selected { background-color: #58a6ff; color: white; border-radius: 3px; }
        QToolBar { background-color: #0d1117; border: none; border-bottom: 1px solid #30363d; spacing: 5px; padding: 5px; }
        
        /* Dynamic Card System */
        QFrame#DataCard { background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 10px; margin-bottom: 5px; }
    ...
    "Light Clean": """
        ...
        QTabWidget::pane { border: 1px solid #d0d7de; background: #ffffff; border-radius: 4px; border-top-left-radius: 0px; }
        QTabBar::tab { background: #f6f8fa; color: #57606a; padding: 8px 15px; border: 1px solid #d0d7de; border-bottom: none; border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: 2px; }
        QTabBar::tab:selected { background: #ffffff; color: #0969da; font-weight: bold; border-top: 2px solid #0969da; }
        QListView::item, QListWidget::item { padding: 5px; border-radius: 5px; }
        ...
        QMenu { background-color: #ffffff; color: #24292f; border: 1px solid #d0d7de; padding: 5px; }
        QMenu::item:selected { background-color: #0969da; color: white; border-radius: 3px; }
        QToolBar { background-color: #f6f8fa; border: none; border-bottom: 1px solid #d0d7de; spacing: 5px; padding: 5px; }
        
        /* Dynamic Card System */
        QFrame#DataCard { background: #ffffff; border: 1px solid #d0d7de; border-radius: 8px; padding: 10px; margin-bottom: 5px; }
        
        /* Calendar System */
        QCalendarWidget QWidget { alternate-background-color: #ffffff; background-color: #f6f8fa; color: #24292f; }
        QCalendarWidget QToolButton { color: #24292f; font-weight: bold; background-color: transparent; padding: 5px; }
        QCalendarWidget QToolButton::hover { background-color: #ebecf0; border-radius: 4px; }
        QCalendarWidget QMenu { background-color: #ffffff; color: #24292f; }
        QCalendarWidget QSpinBox { background: #ffffff; color: #24292f; border: 1px solid #d0d7de; }
        QCalendarWidget QAbstractItemView:enabled { background-color: #f6f8fa; color: #24292f; selection-background-color: #2ea043; selection-color: white; outline: none; }
        QCalendarWidget QAbstractItemView:disabled { color: #8c959f; }
    """
}
...
class NexusDB:
    def __init__(self, path: Path):
        self.path = path
        self.conn = SecurityEngine.connect(str(self.path), uri=True, check_same_thread=False)
...
class MaterializeThread(QThread):
    ...
    def sanitize_filename(self, name):
        # Safely scrubs OS-illegal characters before creating physical folders/files
        return re.sub(r'[\\/*?:"<>|]', '_', str(name))

    def run(self):
        try:
            conn = SecurityEngine.connect(str(self.db_path), uri=True)
            cur = conn.cursor()
            all_exports = [] 
            ...
            for typ, path_val, db_id in self.items:
                ...
                # Exporting a specific individual file
                ...
                # Exporting an entire folder or Smart View
                elif typ == "folder":
                    
                    # --- DYNAMIC SMART VIEW INTERCEPTOR ---
                    if "://" in path_val:
                        prefix, data = path_val.split("://", 1)
                        parts = [p for p in data.split('/') if p]

                        # 1. Year -> Month -> Folders (The y_m_f fix!)
                        if prefix == "y_m_f":
                            if len(parts) == 0: # Root level: Export every year & month
                            ...
                            elif len(parts) == 1: # Year level: Export specifically this year's months
                                yr = parts[0]
                                ...
                                for m, n, rp in res:
                                    ...
                                    all_exports.append((dest, rp))
                            elif len(parts) >= 2: # Month level: Export specifically this month's files
                    ...
                    # --- STANDARD VIRTUAL FOLDER HANDLER ---
            ...
            for i, (rel_dest, source_rp) in enumerate(all_exports):
                ...
                try:
                    # FIX: Safely detects missing/None physical paths and forces 0-byte generation!
                    if source_rp and str(source_rp).strip() not in ("None", "") and os.path.exists(str(source_rp)):
                        shutil.copy2(str(source_rp), final_dest)
                    else:
                        # Spawns a purely virtual 0-byte file into physical reality
                        with open(final_dest, 'w') as f: 
                            pass 
                    count += 1
                except Exception as e:
                    print(f"Materialize Error on {final_dest}: {e}") # Skips the broken file so the loop doesn't crash
...
class BulkHashCalculator(QThread):
    ...
    def run(self):
        try:
            with SecurityEngine.connect(str(self.db_path), uri=True) as conn:
                cur = conn.cursor()
...
class CompilerThread(QThread):
    ...
    def run(self):
        try:
            ...
            src_conn = SecurityEngine.connect(self.source_db)
...
class DuplicateProofDialog(QDialog):
    def __init__(self, db_path, query, params, match_type, main_app, parent=None):
        ...
        with SecurityEngine.connect(db_path) as conn:
            cur = conn.cursor()
            ...
            for f in files:
                ...
                # Route the click back to the main app's native opener
...
class PaginatingChartWidget(QWidget):
    def __init__(self, title, parent=None):
        super().__init__(parent)
        # to check the parent's dark mode state
        self.main_app = parent.main_app if hasattr(parent, 'main_app') else parent         
        self.title = title
        self.full_data = []
        self.current_page = 0
    ...
    def draw_chart(self):
        ...
        if not page_data:
            self.ax.text(0.5, 0.5, "No Data Available", color=text_color, ha='center')
            self.fig.patch.set_facecolor(bg_color); self.ax.set_facecolor(bg_color)
            self.canvas.draw(); return


        # Escape $ signs for the dashboard charts
        keys = [str(k[:30] + '..' if len(k) > 30 else k).replace('$', '\\$') for k, v in page_data]
        ...
        if chart_type != "Pie":
            self.ax.grid(True, axis='x', linestyle='--', alpha=0.3, color=text_color)
            for spine in self.ax.spines.values(): spine.set_color(grid_color)
            
        self.ax.set_title(self.title, color=text_color, pad=15, fontweight='bold')
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Right:
            ...
            if self.level_index + 1 < len(self.parent_dialog.dynamic_lists):
                ...
                # Auto-scroll the window to ensure the new column is perfectly visible
            ...
            # Move focus to the left column, or back to the Tag List if at Level 0
...
class NexusTagLibraryDialog(QDialog):
    ...
    def _setup_shortcuts(self):
        # Jump directly to global column search
    ...
    def refresh_memory_cache(self):
        self.tag_cache.clear()
        try:
            with SecurityEngine.connect(str(self.db_path), timeout=10, uri=True) as conn:
                cur = conn.cursor()
    ...
    def update_analytics_data(self):
        ...
        for idx, (title, val) in enumerate(metrics):
            ...
            # Update to use dynamic base background based on theme
            is_dark = getattr(self.main_app, 'is_dark_mode', True)
            base_bg = "#161b22" if is_dark else "#ffffff"
    ...
    def _add_column(self, level_index, title):
        ...
        # --- Using the new ColumnListWidget ---
        ...
        # ---Triggers instantly as you arrow Up/Down ---
    ...
    def import_csv(self):
        ...
        try:
            with SecurityEngine.connect(self.db_path, timeout=20) as conn:
                cur = conn.cursor()
...
class TimelineDiaryDialog(QDialog):
    def __init__(self, db_path, parent=None):
        ...
        self.diary_browser.setStyleSheet("border: none; padding: 10px;") 
    ...
    def render_charts(self):
        ...
        is_dark = getattr(self.parent(), 'is_dark_mode', True)
        bg_color = '#0d1117' if is_dark else '#ffffff'
        ...
        if not an or an['total'] == 0:
            ...
            ax.set_facecolor(bg_color); ax.text(0.5, 0.5, "No Data", color='#8b949e', ha='center', va='center'); ax.axis('off')
            self.canvas.draw(); return
        ...
        if "Extension" in metric:
            ...
            #  Escape dollar signs so extensions like $RECYCLE don't crash Matplotlib
            safe_labels = [str(x[0]).replace('$', '\\$') for x in sorted_d]
            ax.bar(safe_labels, [x[1] for x in sorted_d], color=colors)
            ...
            # FIXED: Escape dollar signs in filenames like $$.myPC
            names = [str(x[0][:15] + "..." if len(x[0]) > 15 else x[0]).replace('$', '\\$') for x in top_10]
...
class SecurityEngine:
    _instance = None  # Singleton reference for background threads

    def __init__(self, db_path="nexus_vfs.db"):
        self.db_path = db_path
        self.salt_path = db_path + ".salt"  # We must store the salt externally now
        self.raw_hex_key = None
        SecurityEngine._instance = self

    def _derive_raw_key(self, password):
        """
        Derives a 256-bit raw AES key ONCE using Python's C-backend hashlib.
        This prevents background threads from choking the CPU on every connection.
        """
        if not os.path.exists(self.salt_path):
            # Generate a new 32-byte secure salt on first boot
        ...
        # Derive a 32-byte key (600,000 iterations for extreme security)
    ...
    @classmethod
    def connect(cls, path, **kwargs):
        """
        Global Connection Factory.
        Connects instantly using the pre-derived raw hex key.
        """
        kwargs.pop('uri', None)
        conn = sqlite3.connect(str(path), uri=True, **kwargs)
        
        if cls._instance and cls._instance.raw_hex_key:
            # The x'...' syntax tells SQLCipher this is a raw hex key.
            # It completely bypasses the KDF, dropping connection overhead to 0.
            conn.execute(f"PRAGMA key=\"x'{cls._instance.raw_hex_key}'\";")
            conn.execute("PRAGMA cipher_page_size = 4096;")
            
        return conn

    def verify_and_unlock(self, password):
        """
        Tests the derived raw key against the SQLCipher database.
        """
        test_hex_key = self._derive_raw_key(password)
        try:
            test_conn = sqlite3.connect(self.db_path)
            test_conn.execute(f"PRAGMA key=\"x'{test_hex_key}'\";")
            
            test_conn.execute("SELECT count(*) FROM sqlite_master;")
            test_conn.close()
            
            # Success! Store the raw hex key in RAM for instant thread connections
            self.raw_hex_key = test_hex_key
            return True
...
class SecurityLoginBox(QDialog):
    def __init__(self, security_engine, parent=None):
        super().__init__(parent)
        self.engine = security_engine
        
        # If the DB doesn't exist yet, it's the first boot
        self.is_first_run = not os.path.exists(self.engine.db_path)
        
        self.setWindowTitle("Nexus OS - Zero Leakage Edition")
        self.setFixedSize(350, 250 if self.is_first_run else 200)
        self.setStyleSheet(THEMES["Dark Nexus"]) 

        layout = QVBoxLayout(self)
        
        if self.is_first_run:
            layout.addWidget(QLabel("<h2 style='color:#3fb950; text-align:center;'>🛡️ System Initialization</h2>"))
            layout.addWidget(QLabel("<p style='text-align:center; color:#8b949e;'>Create a Master Password for the SQLCipher Engine.<br><b style='color:#f85149;'>If lost, data is unrecoverable.</b></p>"))
        else:
            layout.addWidget(QLabel("<h2 style='color:#f85149; text-align:center;'>🔒 System Locked</h2>"))
            layout.addWidget(QLabel("<p style='text-align:center; color:#8b949e;'>Enter Master Password to decrypt SQLCipher pages.</p>"))

        self.txt_pass = QLineEdit()
        self.txt_pass.setEchoMode(QLineEdit.Password)
        self.txt_pass.setPlaceholderText("Master Password...")
        ...
        if self.is_first_run:
            ...
            self.txt_confirm.setStyleSheet("padding: 8px; border: 1px solid #30363d; border-radius: 4px; background: #161b22; color: white;")
            layout.addWidget(self.txt_confirm)

        self.btn_login = QPushButton("Initialize Encrypted DB" if self.is_first_run else "Decrypt & Boot")
        self.btn_login.setStyleSheet("background-color: #2ea043; color: white; font-weight: bold; padding: 10px; border-radius: 4px; margin-top: 10px;")
    ...
    def attempt_login(self):
        ...
        if self.is_first_run and pwd != self.txt_confirm.text():
            QMessageBox.warning(self, "Mismatch", "Passwords do not match. Please try again.")
            return

        if self.engine.verify_and_unlock(pwd):
            self.accept()
        else:
            QMessageBox.critical(self, "Access Denied", "Fatal Error: Incorrect Master Password.\nDecryption failed.")
            self.txt_pass.clear()
...
class NexusVirtualManager(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # ==========================================
        # 1. NATIVE TOOLBAR (Handles shrinking gracefully!)
        # ==========================================
        ...
        act_new_folder = QAction("📂 Dir", self)
        ...
        act_bulk_del = QAction("🗑 Bulk", self)
        ...
        act_set_storage = QAction("💾 Capacity", self)
        ...
        act_toggle_log = QAction("📝 Con", self)
        ...
        # --- INVISIBLE SPACER (Pushes next items to the far right) ---
        spacer = QWidget()
        ...
        right_controls = QHBoxLayout(right_widget)
        right_controls.setContentsMargins(0, 0, 5, 0)
        ...
        btn_help.setFixedWidth(45)
        
        btn_lock = QPushButton("🔒")
        btn_lock.setStyleSheet("background-color: #f85149; color: white; font-weight: bold; padding: 4px 10px; border-radius: 4px;")
        btn_lock.clicked.connect(self.close) 
        
        right_controls.addWidget(self.theme_combo)
        right_controls.addWidget(btn_help)
        right_controls.addWidget(btn_lock)
        
        tb.addWidget(right_widget) # Add the right zone to the toolbar

        # ==========================================
        # 2. CENTRAL WIDGET & MAIN LAYOUT
        # ==========================================
    ...
    def apply_theme(self, theme_name=None):
        # 1. CRITICAL FIX: Shred the old CSS so colors don't bleed together!
        self.setStyleSheet("") 
        if QApplication.instance():
            QApplication.instance().setStyleSheet("")

        # 2. Get the requested theme
        if theme_name is None: 
            theme_name = self.theme_combo.currentText()
            
        # 3. Apply the fresh theme
        self.setStyleSheet(THEMES.get(theme_name, THEMES["Dark Nexus"]))
        
        # 4. Update internal state and refresh dynamic charts
    ...
    def _on_hash_computed(self, h, db_id, rp):
        if db_id and db_id != -1:
            with SecurityEngine.connect(str(self.db.path), uri=True) as conn:
    ...
    def closeEvent(self, ev):
        # 1. Shut down media and timers cleanly
        if HAS_MULTIMEDIA and hasattr(self, 'player'): self.player.stop()
        ...
        # 2. CRITICAL: Lock and encrypt the database BEFORE closing the connection
        if hasattr(self, 'security_engine') and self.security_engine:
            self.status.showMessage("Encrypting and locking filesystem...", 2000)
            QApplication.processEvents() 
            self.security_engine.lock_database()

        # 3. Terminate the main SQLite connection
        self.db.close()
        super().closeEvent(ev)
    ...
    def closeEvent(self, event):
        ...
        # 1. Force close the persistent main database connection if it exists
        if hasattr(self, 'conn') and self.conn:
            try: self.conn.close()
            except: pass
            
        # 2. Force Python to destroy any unclosed background SQLite cursors
        gc.collect()
...
if __name__ == "__main__":
    ...
    security = SecurityEngine("virtual_fs.db")
    
    # 2. Halt boot and demand password
    login = SecurityLoginBox(security)
    if login.exec() == QDialog.Accepted:
        
        # 3. Pass the secure DB path to the global environment
        DB_FILE = security.db_path
        
        window = NexusVirtualManager()
        
        # Attach the security engine so the app knows how to lock itself on close
        window.security_engine = security 
